...
>
> **공용(구글) 동기화 · 접속자 현황**: 전용 구글시트를 공용 저장소로 두면, 팀(4명)이 **같은 설정을 공유**하고 캐시가 지워져도 **[공용 설정 받기]** 로 복원됩니다. 상단바에 **실시간 접속자 수**도 표시됩니다. 무거운 데이터(상담·발주)는 각자 시트에 쌓이고 여기엔 작은 설정·하트비트만 저장하므로 **구글 무료 한도로 4명은 충분**합니다. 설치 코드: [`google-apps-script-sync.gs`](google-apps-script-sync.gs) → 상단바 [설정 백업] → “공용(구글) 동기화”에 URL 입력.
...
assets/
  ...
  js/   config.js          접속코드·플랫폼 프리셋·내비  ← 실서비스 전환 시 교체 지점
        ...
        sync.js            공용 저장소(구글) 클라이언트 — 팀 설정 공유 + 접속자 현황
