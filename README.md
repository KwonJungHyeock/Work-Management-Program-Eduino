...
- **토글 버튼**: 분류([후불][견적][상품/재고][주문/배송][기타]) · 고객유형([학교][개인][기관][업체][입점사][파트너사]) · 상품분류([자사키트][자사부품][입점사키트][입점사부품])를 **전화하면서 바로 클릭**. 분류는 화면에서 추가/삭제 편집 가능. 저장 시 폼 즉시 초기화(분류·상담사·날짜 유지).
- **오늘 기록**: 최신순 카드, 분류 필터, 수정/삭제, 동기화 상태 배지.
- **일일 결산**: 날짜별 총건수·**분류별·고객유형별·상담사별** 집계 + 콜백 필요 목록, 결산 텍스트 복사.
