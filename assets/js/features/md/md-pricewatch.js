    ...
    render(root){
      ...
      function drawAlerts(sec,v){ const prods=effectiveProducts(v);
        ...
        sec.innerHTML=`${apDay?`<div class="nx-note">최근 확인: <b>${esc(apDay)}</b> · 변동 <b>${(diffs||[]).length}</b>건${hidden?` · 처리/삭제 <b>${hidden}</b>건`:''}
            <div style="font-size:11.5px;color:var(--muted);margin-top:4px">${icon('info')||''} <b>검수 사이클</b> — ${esc(v.siteName)} 판매가를 <b>매일 오전 10시</b> 자동 수집. 전체 상품을 <b>3일 주기</b>로 나눠 순환 점검(하루 약 200개)하여 각 상품을 <b>3일마다</b> 최신화합니다.</div></div>`:
          `<div class="nx-note">${icon('alert')} 아직 크롤러가 연동되지 않아 <b>예시 미리보기</b>입니다. <b>검수 사이클</b>: 크롤러가 ${esc(v.siteName)} 판매가를 <b>매일 오전 10시</b> 수집(전체 <b>3일 주기</b> 순환·하루 약 200개)하면 실제 변동분이 자동 표시됩니다.</div>`}
          <div class="nx-kpi">
