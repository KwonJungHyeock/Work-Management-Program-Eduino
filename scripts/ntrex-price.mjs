...
function extractPrice(html){
  ...
  const pick = c.find(x=>x.tag==='jsonld'||x.tag==='meta') || c.find(x=>x.tag==='label') || c[0];
...
if(ONE){ items=[{ ntx:String(ONE), ed:'', name:'', basePrice:0 }]; }
  ...
  let list; try{ list=JSON.parse(readFileSync(new URL('./ntrex-list.json', import.meta.url),'utf8')); }
  catch(e){ console.error('ntrex-list.json not found. Put it in the same folder. (single code check works without it: --code=NUMBER)'); process.exit(1); }
  items=list.filter(p=>p.ntx); if(LIMIT) items=items.slice(0,LIMIT);
}
